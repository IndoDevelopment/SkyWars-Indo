...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
        if(!$this->data["enabled"]) {
            $player->sendMessage("§c§lERROR!§r§c Arena sedang dalam pembuatan!");
            return;
        }

        if(count($this->players) >= $this->data["slots"]) {
            $player->sendMessage("§c§lERROR!§r§c Arena sudah penuh!");
            return;
        }

        if($this->inGame($player)) {
            $player->sendMessage("§c§lERROR!§r§c Kamu sudah berada diarena!");
            return;
        }
        ...
        $player = $player->getNameTag();
        $this->broadcastMessage("§e$player §amemasuki arena");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $player = $player->getNameTag();
            $this->broadcastMessage("§e$player §ckeluar dari arena");
        }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("§aMULAI!", self::MSG_TITLE);
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME) {
            $player->sendMessage("§c§lERROR!§r§c Arena sudah dimulai!");
            return;
        }
        if($this->phase == self::PHASE_RESTART) {
            $player->sendMessage("§c§lERROR!§r§c Arena sedang direstart!");
            return;
        }
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player, "Kamu berhasil keluar dari arena!");
        }
    }
